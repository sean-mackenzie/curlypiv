...
# --- Load file paths ---
dirPath = '/Users/mackenzie/Desktop/ACEO/06.29.21 - ACEO'
gridPath = None
flatfieldPath = None
...
pt_bpe = material_solid(name='Pt', transparency=0.01, conductivity=22e-9)        # Ref: 4/6/21, 30 nm Au - 75% @ 50X, 51% @ 20X
...
kcl = material_liquid(name='KCl', species='KCl', conductivity=18e-4, concentration=0.1, pH=6.2, density=1000, viscosity=0.001, permittivity=80.1, temperature=298) # Ref: 3/4/21 BNL Conductivity Meter + Zetasizer
...
# physical
fluoro_particles = fluorescent_particles(diameter=870e-9, concentration=2e-5, materials=polystyrene, electrophoretic_mobility=-1.47e-8)    # Needs to be measured. I used 3X the 800 nm particles.
bpe_iceo_channel = channel(length=20e-3, width=5e-3, height=20.0e-6, material_bottom_wall_surface=sio2_channel, material_top_wall_surface=sio2_channel, material_fluid=kcl)                # Ref: 3/11/21, NCF Dektak of PDMS channel height
bpe_iceo_bpe = bpe(length=500e-6, width=500e-6, height=20e-9, material=pt_bpe, adhesion_material=alkane_thiol, dielectric_coating=no_dielectric)                            # Ref: 4/6/21, Brightfield BPE images + NCF Dektak
bpe_iceo_electrode_config = electrode_configuration(material='Platinum', length=bpe_iceo_channel.length, entrance_length=1e-6)
# optics
...
darkfield = darkfield(basePath=dirPath, flip_image_across_axis=None, show_image=darkfield_show_image, save_image=darkfield_save_image, save_plot=darkfield_save_plot)
emccd = ccd(exposure_time=.02882, img_acq_rate=34.376, EM_gain=5, darkfield=darkfield)
...
x20_objective = objective(name='LCPLFLN20xLCD', fluoro_particle=fluoro_particles, illumination=flatfield, basePath=dirPath, channel_height=bpe_iceo_channel.height, show_depth_plot=obj_show_depth_plot, save_depth_plot=obj_save_depth_plot)
microscope = microscope(type='Olympus iX73', objective=x20_objective, illumination=flatfield, ccd=emccd)
# higher-level
bpe_iceo_optics = optics(microscope=microscope, fluorescent_particles=fluoro_particles, calibration_grid=None, pixel_to_micron_scaling=x20_objective.pixel_to_micron)
...
bpespecs = {
    ...
    'bxmin': 30,  # x = 0 is the left of the image
    'bxmax': 150,
    'bymin': 25,
    'bymax': 450,  # y = 0 is the bottom of the image
    'multiplier': 1.0
}
cropspecs = {
    ...
    'xmax': 190,
    'ymin': 0,
    'ymax': 500  # y = 0 is the bottom of the image
}
filterspecs = {
