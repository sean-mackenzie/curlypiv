...
# --- Load file paths ---
dirPath = '/Users/mackenzie/Desktop/BPE-ICEO/05.27.21-bpeiceo-bareAu'
gridPath = None
flatfieldPath = None
...
plot_flatfield_correction = True
plot_background_subtraction = True
# filtering
plot_filter_example = True
# examine testset
...
pdms_channel = material_solid(name='PDMS', zeta=-0.005, permittivity=2.5, index_of_refraction=1.4, conductivity=4e-13, thickness=2e-3, youngs_modulus=500e3, poissons_ratio=0.5, density=0.97, dielectric_strength=20e6)                  # Ref: <http://www.mit.edu/~6.777/matprops/pdms.htm>
...
# physical
fluoro_particles = fluorescent_particles(diameter=180e-9, concentration=2e-5, materials=polystyrene, electrophoretic_mobility=-9e-8)    # Needs to be measured. I used 3X the 800 nm particles.
bpe_iceo_channel = channel(length=20e-3, width=550e-6, height=39.0e-6, material_bottom_wall_surface=sio2_channel, material_top_wall_surface=pdms_channel, material_fluid=kcl)                # Ref: 3/11/21, NCF Dektak of PDMS channel height
...
bpespecs = {
    ...
    'bxmin': 65,  # x = 0 is the left of the image
    'bxmax': 380,
    'bymin': 16,
    'bymax': 334,  # y = 0 is the bottom of the image
    'multiplier': 2.0
}
cropspecs = {
    'xmin': 7,  # x = 0 is the left of the image
    'xmax': 446,
    'ymin': 0,
    'ymax': 351  # y = 0 is the bottom of the image
}
filterspecs = {
    'denoise_wavelet': {'args': [], 'kwargs': dict(method='BayesShrink', mode='soft', rescale_sigma=True)},
    'median': {'args': [disk(3)]},
    'gaussian': {'args': [1.5]},
    #'equalize_adapthist': {'args': [], 'kwargs': dict(kernel_size=int(np.round(bpe_iceo_bpe.length*1e6/x50_objective.pixel_to_micron/7)))},
...
testCol = CurlypivTestCollection(collectionName=name, dirpath=dirPath, file_type=img_type, testid=testid,
                                 runid=runid, seqid=seqid, frameid=frameid, load_files=load_files, file_lim=50,
