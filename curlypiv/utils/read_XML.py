...
def read_Squires_XML(return_rows=10, xml_path=None, write_to_disk=False, write_path=None, **kwargs):

    # define the XML filepath
    if xml_path is None:
        xml_path = '/Users/mackenzie/PythonProjects/curlypiv-master/curlypiv/data/raw/squires/Pascall_and_Squires_2009_experimental_data.xml'
    if write_path:
        save = write_path
        save_sampled = write_path[:-4] + '_sampled' + '.csv'
    else:
        save_path = '/Users/mackenzie/PythonProjects/curlypiv-master/curlypiv/data/results'
        save_name = 'Pascall_and_Squires_2009_results.csv'
        save_sampled_name = 'Pascall_and_Squires_2009_results_sampled.csv'
        save = join(save_path, save_name)
        save_sampled = join(save_path, save_sampled_name)
    ...
    if kwargs:
        for arg, minmax in kwargs.items():  # filter rows by kwargs
            if len(minmax) > 1:
                df = df.loc[(df[arg] >= minmax[0]) & (df[arg] <= minmax[1])]
            else:
                df = df.loc[df[arg] == minmax[0]]
        df.sample(n=return_rows)    # return random sample of rows

        if write_to_disk is True:
            df.to_csv(path_or_buf=save_sampled, header=columns, index=False)
