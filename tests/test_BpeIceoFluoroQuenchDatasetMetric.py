...
from curlypiv.CurlypivTestSetup import material_liquid, material_solid, bpe, channel, fluorescent_particles, ccd, \
    objective, microscope, illumination, darkfield
from curlypiv.datasets import BpeIceoActuatorDataset
from curlypiv.metrics import BpeIceoActuatorDatasetMetric

# some initial path variables
base_directory = '/Users/mackenzie/Desktop/06.15.21 - BPE fluoro'
darkfield_path = '/Users/mackenzie/Desktop/06.15.21 - BPE fluoro/darkfield/test_1_X1.tif'
...
# components
fluoro_particles = fluorescent_particles(diameter=870e-9, concentration=2e-5, materials=polystyrene,
                                         electrophoretic_mobility=-4e-8)  # Needs to be measured. I used 3X the 800 nm particles.
bpe_iceo_channel = channel(length=20e-3, width=550e-6, height=39.0e-6, material_bottom_wall_surface=sio2_channel,
                           material_top_wall_surface=sio2_channel,
...
ccd_darkfield = darkfield(basePath=base_directory,  darkframePath=darkfield_path, flip_image_across_axis=False, show_image=False, save_image=False, save_img_type='.tif',
                                      savePath=None, savename=None, save_plot=False)

emccd = ccd(exposure_time=.02265, img_acq_rate=43.611, EM_gain=4, darkfield=None)
...
# MEMBRANE (is a CurlypivTestSetup.material_solid class object)
MEMBRANE = material_solid(name='Silpuran', length=10e-3, width=650e-6, thickness=20e-6,
                          zeta=-0.025, permittivity=2.5, index_of_refraction=1.4,
                              conductivity=4e-13, youngs_modulus=500e3, poissons_ratio=0.5,
                              density=0.97, dielectric_strength=20e6)  # Ref: silpuran datasheet
...
bpeiceoDataset = {
    "bpe": BPE,
    "dielectric_coating": NO_DIELECTRIC,
    "channel": bpe_iceo_channel,
    "buffer": kcl,
    "microscope": microscope,
    "ccd": emccd,
...
RESULTS_DIR = '/Users/mackenzie/Desktop/BPE-ICEO/06.08.21 - BPE-ICEO actuator/results/calibration'
PATHS = ['test_calib_100imagesperstep.csv']
FIELDS = np.ones_like(PATHS, dtype=int) * 20e3
FREQS = [150, 500, 250, 100, 75, 0.1, 100]
WAVEFORMS = ['sine', 'sine', 'sine', 'sine', 'sine', 'square', 'sine']

bpeiceoactuatorDataset = BpeIceoActuatorDataset(membrane=MEMBRANE, **bpeiceoDataset)

for i in range(len(PATHS)):
    PATH = join(RESULTS_DIR, PATHS[i])
    bpeiceoactuatorMetrics = BpeIceoActuatorDatasetMetric(dataset=bpeiceoactuatorDataset,
                                                          particle_tracking_data_filepath=PATH,
                                                          electric_field_strength=FIELDS[i],
                                                          frequency=FREQS[i],
                                                          waveform=WAVEFORMS[i])

    print(bpeiceoactuatorMetrics)
    print(PATH)
    j=1
...
PATHS = ['f150Hz_substack_150-350.csv', 'f500Hz_substack_150-350_coords.csv', 'f250Hz_substack_150-350_coords.csv',
        'f100Hz_substack_200-400_coords.csv', 'f75Hz_substack_150-350_coords.csv', 'f100mHz_square_substack_coords.csv',
         'f100Hz_substack_coords.csv']
"""